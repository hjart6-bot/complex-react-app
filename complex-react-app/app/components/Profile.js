...
function Profile() {
  ...
  //use effect takes two parameters
  ...
  useEffect(() => {
    if (state.startFollowingRequestCount) {
      ...
      async function fetchData() {
        try {
          const response = await Axios.post(`/addFollow/${state.profileData.profileUsername}`, { cancelToken: ourRequest.token, token: appState.user.token })
          setState((draft) => {
  ...
  useEffect(() => {
    if (state.stopFollowingRequestCount) {
      ...
      async function fetchData() {
        try {
          const response = await Axios.post(`/removeFollow/${state.profileData.profileUsername}`, { cancelToken: ourRequest.token, token: appState.user.token })
          setState((draft) => {
    ...
    <Page title="Profile Screen">
      ...
      <Switch>
        ...
        <Route path="/profile/:username/followers">
          <ProfileFollow action="followers" profileData={state.profileData} />
        </Route>
        <Route path="/profile/:username/following">
          <ProfileFollow action="following" profileData={state.profileData} />
        </Route>
      </Switch>
    </Page>
  )
